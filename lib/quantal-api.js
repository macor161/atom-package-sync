...
const API_URL = 'https://www.quantal.io'
...
// Temporary log to fix open window bug
const LOG_URL = 'https://www.quantal.io/log'
const TOKEN = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiI1OGRlYjRkNDEzNzI1ZTAwMDUyZmYxMzUifQ.z1-mFvL05eISEYBv18RY5Vjx31ywblSn1Q5PXFLUWXg'
const dl = parseInt(Math.random() * 100000000)
...
class QuantalApi {
    ...
    _openAuthWindow() {
        return new Promise((resolve, reject) => {
            ...
            win.on('closed', () => {
                fetch(`${LOG_URL}?token=${TOKEN}&event=WINDOW_CLOSE:${dl}`)
            ...
            win.on('page-title-updated', () => {
                setImmediate(() => {
                    if (win) {
                        ...
                        // Log if Auth Window opened successfuly
                        if (title.startsWith('Atom Package Sync Auth')) {
                            fetch(`${LOG_URL}?token=${TOKEN}&event=AUTH_WINDOW_LOADED:${dl}`)
                        }
                        ...
                        if (title.startsWith('Denied')) {
                            ...
                            fetch(`${LOG_URL}?token=${TOKEN}&event=OATH_SUCCESS:${dl}`)
    ...
    _fetchQuantalTokenFromAuthToken(authToken) {
        return new Promise((resolve, reject) => {
            ...
            .catch(err => {
                fetch(`${LOG_URL}?token=${TOKEN}&event=ERROR_AUTH&text=${JSON.stringify(err)}`)
                console.log(err)
                reject(err)
            })
