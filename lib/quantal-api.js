...
class QuantalApi {
    ...
    fetchAtomSettings() {
        return new Promise((resolve, reject) => {
            ...
            .then(token => fetch(`${this._apiUrl}/package-sync/settings?token=${token}`))
    ...
    fetchAtomSettingsInfo() {
        return new Promise((resolve, reject) => {
            ...
            this._getQuantalToken()
                .then(token => fetch(`${this._apiUrl}/package-sync/lastUpdate?token=${token}`) )
                .then(response => response.json())
                .then(result => {
                    if (!result.error) {
    ...
    _openAuthWindow() {
        return new Promise((resolve, reject) => {
            ...
            win.on('closed', () => {
                reject(new QError(QuantalError.CONNECT_WINDOW_CLOSED, 'User closed the connexion window'));
            });
...
var QError = function(name, message, fileName, lineNumber) {
...
module.exports = { QuantalApi, QuantalError, QError}
