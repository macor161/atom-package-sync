...
class QuantalApi {
    ...
    fetchAtomSettings() {
        return new Promise((resolve, reject) => {
            ...
            .then(token => { _token = token; return fetch(`${this._apiUrl}/package-sync/settings?token=${token}`, { retries: FETCH_RETRIES, retryDelay: FETCH_RETRY_DELAY }) })
            ...
            .catch(err => {
                console.log(err)
                fetch(`${this._apiUrl}/package-sync/settings?token=${_token}`, { retries: 1, retryDelay: FETCH_RETRY_DELAY })
    ...
    fetchAtomSettingsInfo() {
        return new Promise((resolve, reject) => {
            ...
            var _token
            this._getQuantalToken()
                .then(token => { _token = token; return fetch(`${this._apiUrl}/package-sync/lastUpdate?token=${token}`, { retries: FETCH_RETRIES, retryDelay: FETCH_RETRY_DELAY }) })
                .then(response => response.json())
                .then(result => {
                    if (!result.error) {
    ...
    /**
     * Remove Quantal and auth user token from cache
     *
     * @return {Promise<string>} Remove the token
     */
    logoutUser() {
      return new Promise((resolve, reject) => {
        ...
        var googleToken = store.get('atom-package-sync:googleToken');

        if(googleToken)
          store.remove('atom-package-sync:googleToken');
